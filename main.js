...
//let initialGameState = "TEST-GAME-AREA"; //"GAME-SETUP";
...
function preload() {
    partyConnect(
        ...
        "jkv-strategoCoreV12h", 
        room
    );

    shared = partyLoadShared("shared", {
        gameState: initialGameState, //"GAME-SETUP",
        winningTeam: null,
        resetFlag: false,
...
function draw() {
    ...
    if (shared.gameState === "TEST-GAME-AREA") {
        ...
        if (firstRun) {
            ...
            me.characterId = "C"; // Set characterId to "C" for Core Commander in TEST-GAME-AREA
            me.characterInstanceId = "blue_C_0"; // Set characterInstanceId to "C-0" for Core Commander in TEST-GAME-AREA
            me.characterName = "Core Commander"; // Set characterName to "Core Commander" for TEST-GAME-AREA
            me.characterRank = -1
            me.diameter = 60
            me.hasCharacter = true; // Set hasCharacter to true for TEST-GAME-AREA
            me.isReady = true; // Set isReady to true for TEST-GAME-AREA
            me.lastProcessedResetFlag = false; // Initialize lastProcessedResetFlag
            me.playerColor = 'purple'; // Set playerColor to purple for TEST-GAME-AREA       
            me.playerDisplayName = "Player123"
            me.playerName = "player0"; // Set playerName to "Player123" for TEST-GAME-AREA
            me.size = 60
            me.status = "available"; // Set status to "available" for TEST-GAME-AREA
            me.team = "blue"; // Set team to "blue" for TEST-GAME-AREA
            me.xGlobal = 0
            me.xLocal = 200
            me.yGlobal = 0
            me.yLocal = 500
    ...
    //if (shared.gameState === "TEST-GAME-AREA") {
    //    text("xGlobal: + me.yGlobal, 100, 10);
    //}
    ...
    text("me.mouseX: " + me.xMouse + " me.yMouse: " + me.yMouse, 300, 10);
...
function createSpacecrafts() {
    for (let i = 0; i < TOTAL_NUMBER_OF_PLAYERS; i++) {
        ...
        spacecrafts.push(new Spacecraft({
            ...
            xGlobal: 0,  //3000 / 2 - GAME_AREA_WIDTH / 2 + 400,
            yGlobal: 0, //3500 / 2 - GAME_AREA_HEIGHT / 2,
...
function spawn(spacecraft) {
    ...
    me.xGlobal = 0; //spacecraft.xGlobal;
    me.yGlobal = 0; //spacecraft.yGlobal;
...
function drawWarpGateAnimation(warpGateGlobalX, warpGateGlobalY, localOffsetX, localOffsetY, imageFrames, frameWidth, frameHeight, speedDivisor, isPulsating, isCooldown = false) {
    ...
    //    let yLocal = warpGateGlobalY - selectedPlanet.yCropOffset - me.yGlobal + localOffsetY;
...
*/
        ...
        if (showBlurAndTintEffects && !imagesStillLoading) {
            ...
            // Draw the animation for planet 0's down warp gate  
            if (me.planetIndex === 0) {
                text("selectedPlanet.xWarpGateUp: " + selectedPlanet.xWarpGateUp + " selectedPlanet.yWarpGateUp: " + selectedPlanet.yWarpGateUp, 1500, 10);
                text("selectedPlanet.xWarpGateDown: " + selectedPlanet.xWarpGateDown + " selectedPlanet.yWarpGateDown: " + selectedPlanet.yWarpGateDown, 1500, 30);
        ...
        drawRadialGradient(
            //            GAME_AREA_X - me.xGlobal + selectedPlanet.diameterPlanet / 2,
            //            GAME_AREA_Y - me.yGlobal + selectedPlanet.diameterPlanet / 2,
            GAME_AREA_X + selectedPlanet.diameterPlanet / 2,
            GAME_AREA_Y + selectedPlanet.diameterPlanet / 2,
            selectedPlanet.diameterPlanet,
            colorScheme.center,
            colorScheme.edge
        );
...
function mousePressed() {
    ...
    let bullet = {
        ...
        xStart: me.xLocal,
        yStart: me.yLocal,
        //        xMouseStart: mouseX - GAME_AREA_X,  // Use current mouse position
        //        yMouseStart: mouseY - GAME_AREA_Y + selectedPlanet.yCropOffset,  // Use current mouse position
        xMouseStart: me.xMouse,  // Use current mouse position
        yMouseStart: me.yMouse,  // Use current mouse position
        xGlobal: 0,
        yGlobal: 0,
    };
    console.log(me)
...
function setSpawnLocation() {
    ...
    if (!selectedPlanet) {
        ...
        me.xGlobal = 0; //SCREEN_WIDTH / 2; // Or some other sensible default 2
        me.yGlobal = 0; //SCREEN_HEIGHT / 2;
    ...
    if (me.team === 'blue') {
        ...
        me.xGlobal = 0; //selectedPlanet.xWarpGateDown - 400;
        me.yGlobal = 0; //selectedPlanet.yWarpGateDown - 200;
        //        me.xLocal = GAME_AREA_X + 200;
        //        me.yLocal = GAME_AREA_Y + selectedPlanet.yCropOffset + 600;
        ...
        me.xGlobal = 0//selectedPlanet.xWarpGateUp - GAME_AREA_WIDTH / 2 - 200;
        me.yGlobal = 0//selectedPlanet.yWarpGateUp - GAME_AREA_HEIGHT / 2 - 100;
        //me.xLocal = GAME_AREA_X + 600;
        //me.yLocal = GAME_AREA_Y + selectedPlanet.yCropOffset + 400;
        me.xLocal = selectedPlanet.xWarpGateDown - 100;
        me.yLocal = selectedPlanet.yWarpGateDown - 100;
    }
    /*
} else if (me.planetIndex === 1) {
    if (me.team === 'blue') {
        // Blue team spawns 100 pixels to the right of the 'up' warp gate
        me.xGlobal = 0; //selectedPlanet.xWarpGateDown - 400;
        me.yGlobal = 0; //selectedPlanet.yWarpGateDown - 200;
    ...
    if (me.team === 'blue') {
        ...
        // Preserve existing local coordinate setup for green team
        me.xGlobal = 0;//selectedPlanet.xWarpGateDown - GAME_AREA_WIDTH / 2 - 200;
        me.yGlobal = 0;//selectedPlanet.yWarpGateDown - GAME_AREA_HEIGHT / 2 - 100;
        me.xLocal = GAME_AREA_WIDTH / 2 - 200;
        me.yLocal = GAME_AREA_HEIGHT / 2 - 100;
    }
} else {
    if (me.team === 'blue') {
        // Blue team spawns 100 pixels to the right of the 'up' warp gate
        me.xGlobal = 0;//selectedPlanet.xWarpGateUp - 400;
        me.yGlobal = 0;//selectedPlanet.yWarpGateUp - 200;
        me.xLocal = GAME_AREA_WIDTH / 2 + 200;
        me.yLocal = GAME_AREA_HEIGHT / 2 + 100;
...
function handleBulletMovement() {
    ...
    for (let i = me.bullets.length - 1; i >= 0; i--) {
        ...
        //console.log(bullet)
        //        console.log("xLocal", bullet.xLocal, "yLocal", bullet.yLocal);
...
function updateTowerCount() {
    ...
    shared.gameObjects = gameObjects.map(tower => ({
        ...
        angle: 0,
        hits: Array(15).fill(0),
        planetIndex: 0, // Set to planet 3 specifically
        lastShotTime: 0,
...
function generateTowers() {
    ...
    const towerTable = [
        { x: 400, y: 400 + 450, color: 'red', type: 0 },
        { x: 500, y: 500 + 450, color: 'blue', type: 1 },
        { x: 600, y: 600 + 450, color: 'green', type: 2 },
    ];
